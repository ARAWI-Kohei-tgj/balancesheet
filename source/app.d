...
void main(in string[] argMain) @system{
	...
	(Connection conn) @system{
		...
		const string tableNameRef= (calcPeriod.st.month is periodStartMonth)? "balance_terminal": "balance_monthly";
...
+/
   ...
   ***********************************************************/
	if((mode is ActionMode.trial && calcPeriod.st.month is periodStartMonth) ||
		...
		@(DatabaseAccess.insert) QueryParams procTerminal;
		with(procTerminal){
			sqlCommand= `INSERT INTO special_process
...
($1::INTEGER, $2::TEXT, $3::INTEGER, $4::TEXT, $5::TEXT);`;
		...
		CAPITAL_UPGRADING:
			...
			with(procTerminal){
				args[0]= toValue(calcPeriod.st.toISOExtString);	// FIXME:
			...
			values["元入金"].balanceLast += values["事業主借"].balanceLast -values["事業主貸"].balanceLast
				+profitLast;
		...
		INVENTORY_TRANSFER_BEG:
			const string[4] ingredient= () @safe pure nothrow{
				...
				result[].each!(a => assert(accountTitles[].canFind(a)));
				return result;
			}();
...
WHERE proc_date = $1::DATE;`;
				const Date processDate= (calcPeriod.st.month is periodStartMonth)?
					calcPeriod.st: calcPeriod.en -dur!"days"(1);
				@(DatabaseAccess.readonly) QueryParams cmd;
				with(cmd){
...
+/
   ...
   *******************************************/
	{
		// 登録先のtable名
		const string tableNameRgstr= (mode is ActionMode.trial)? "balance_monthly": "balance_terminal";
		const Date closingDate= calcPeriod.en -dur!"days"(1);
...
WHERE closing_date >= $2::DATE
  AND closing_date < $3::DATE;`;
			...
			(Connection conn, in Date theDate) @system{
				enum string QUERY_TRIAL= `INSERT INTO balance_monthly
(title, closing_date, balance) VALUES
($1::TEXT, $2::DATE, $3::INTEGER);`;
				enum string QUERY_CLOSED= `INSERT INTO balance_terminal
(title, closing_date, balance) VALUES
($1::TEXT, $2::DATE, $3::INTEGER);`;

				@(DatabaseAccess.insert) QueryParams cmdExistance, cmdReg;
				size_t idx= 1;
...
($1::DATE, $2::INTEGER);`;
	auto buf= appender!string;
	@(DatabaseAccess.insert) QueryParams cmd;
